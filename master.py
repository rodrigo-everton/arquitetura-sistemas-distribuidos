...
HOST         = CFG["server"]["ip"]
PORT         = int(CFG["server"]["port"])           # porta Master<->Master
WORKER_PORT  = PORT + 1                              # porta Master<->Worker
SERVER_UUID  = HOST                                  # usamos IP como "id" p/ simplificar o protocolo
...
# carga fake opcional (desligada por padrão)
...
def send_json(conn, obj):
    try:
        ...
        logging.info(f"[PAYLOAD→{conn.getpeername()[0]}] {obj}")
        ...
        while True:
            ...
            if b"\n" in buf:
                ...
                try:
                    ...
                    logging.info(f"[PAYLOAD←{conn.getpeername()[0]}] {data}")
                    return data
                except Exception:
                    return None
    except Exception:
...
def send_alive_master():
    while True:
        for peer in PEERS:
            ip, port = peer["ip"], int(peer.get("port", PORT))
...
def _borrow_plan():
    ...
    # Regra simples: peça 1 de cada peer vivo
...
def initiate_worker_release(owner_server_uuid, workers_list):
    owner_ip = owner_server_uuid  # neste desenho SERVER_UUID == IP
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(5)
...
def command_worker_redirect(workers_list, dest_ip, dest_port):
    ...
    for wid in workers_list:
        try:
            with lock:
                if wid not in workers_controlled:
                    continue
                wip = workers_controlled[wid]
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.settimeout(5)
                s.connect((wip, WORKER_PORT))
    ...
    try:
        ...
        if task == "HEARTBEAT":
            ...
            if chosen:
                ...
                send_json(c, resp)
                # REDIRECT assíncrono para cada worker escolhido
                def _redir():
                    for u, _ in chosen:
                        try:
                            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as ws:
                                ws.settimeout(5)
                                ws.connect((workers_controlled[u], WORKER_PORT))
...
def receive_alive_worker(conn, addr):
    try:
        ...
        # se este worker está retornando para o dono, finalize protocolo
        with lock:
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # apenas loga o retorno (STATUS OK/NOK) — protocolo simples
            except Exception:
                pass
            time.sleep(0.2)
    finally:
